...
namespace WPXMaintenanceProLight\WPBones\Foundation;

use WPXMaintenanceProLight\WPBones\Container\Container;
use WPXMaintenanceProLight\WPBones\Contracts\Foundation\Plugin as PluginContract;
use WPXMaintenanceProLight\WPBones\Database\WordPressOption;
use WPXMaintenanceProLight\WPBones\Foundation\Http\Request;
use WPXMaintenanceProLight\WPBones\Foundation\Log\LogServiceProvider;
use WPXMaintenanceProLight\WPBones\Support\Str;
use WPXMaintenanceProLight\WPBones\View\View;
...
class Plugin extends Container implements PluginContract
{
    ...
    public function boot()
    {
         ...
         *
          ...
          Description = "WPX Maintenance Pro Light is a WP Bones boilperate plugin"
          DomainPath = "localization"
          Name = "WPX Maintenance Pro Light"
          Network = false
          PluginURI = "http://undolog.com"
          TextDomain = "wpx-maintenance-pro-light"
          Title = "WPX Maintenance Pro Light"
          Version = "1.0.0"
        ...
        // Load text domain
        load_plugin_textdomain("wpx-maintenance-pro-light", false, trailingslashit(basename($this->basePath)) . $this->pluginData['DomainPath']);
     ...
     *
     * @return \WPXMaintenanceProLight\WPBones\View\View
     */
    ...
    public function js($filename, $deps = [], $version = null, $footer = true)
    {
        ...
        foreach ($filenames as $file) {
            wp_enqueue_script(
                $this->slug . Str::slug($file),
                WPXMaintenanceProLight()->js . '/' . $file,
                (array) $deps,
                $version??$this->Version,
                $footer
            );
                    ...
                    if (isset($subMenu['route']['load'])) {
                        ...
                        add_action(
                            ...
                            function () use ($controller, $method) {
                                $className = "WPXMaintenanceProLight\\Http\\Controllers\\{$controller}";
                                $instance  = new $className;
